...
DanceApp::DanceApp() : GraphicsApp(1280,768, "So You Think Ants Can Dance") {
    ...
    ballet_special4_.TrimFront(360);
    ballet_special4_.TrimBack(120);
    ...
    ballet_special5_.TrimFront(180);
...
void DanceApp::OnSpecialKeyDown(int key, int scancode, int modifiers) {
    ...
    if (key == GLFW_KEY_KP_1) {
        DanceApp::OnMotion1BtnPressed();
    }
    if (key == GLFW_KEY_KP_2) {
        DanceApp::OnMotion2BtnPressed();
    }
    if (key == GLFW_KEY_KP_3) {
        DanceApp::OnMotion3BtnPressed();
    }
    if (key == GLFW_KEY_KP_4) {
        DanceApp::OnMotion4BtnPressed();
    }
    if (key == GLFW_KEY_KP_5) {
        DanceApp::OnMotion5BtnPressed();
    }
    if (key == GLFW_KEY_SPACE) {
        if (!floor_change_) {
            floor_tex_.InitFromFile(Platform::FindFile("dancefloor.png", searchPath_));
            floor_change_ = true;
            ballet_ant_.change_party();
            salsa_ant_male_.change_party();
            salsa_ant_female_.change_party();
        }
        else {

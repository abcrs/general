...
public:
    ...
    /// Special keys to replicate buttons with keyboard
...
private:
    ...
    // boolean values to change the shape drawn
    bool sphere_;
    bool morphing_;

    // Values to help smoothly transition from flat to globe and back
    double morph_step_;

    // Values to spin the globe
    double rotation_speed_;
    double rotation_;

    // Value for intensity of earthquake images
    double size_intensity_;

    // Variables for drawing earthquakes
    std::vector<Earthquake> visible_quakes_;
    int quake_index_ = 250;
    float mag_range_;

...
class Party_Data {
 ...
 Party_Data(char name, int v, int s, int nc, Candidate_Data * cands) :
  ...
  Party_Data& operator=(const Party_Data &rhs) {  // overload assignment operator
    if (this != &rhs) {               // do not copy to yourself
      name_ = rhs.name_;
      votes_ = rhs.votes_;
      seats_ = rhs.seats_;
      candidates_ = rhs.candidates_;
      num_candidates_ = rhs.num_candidates_;
      rand_seed_ = rhs.rand_seed_;
    }

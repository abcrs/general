...
ImageEditor::mblur_dir_names_ = {
  {ImageEditor::MBLUR_DIR_N_S, "North/South"},
  {ImageEditor::MBLUR_DIR_E_W, "East/West"},
  {ImageEditor::MBLUR_DIR_NE_SW, "Northeast/Southwest"},
  {ImageEditor::MBLUR_DIR_NW_SE, "Northwest/Southeast"}};
...
void ImageEditor::ApplyChannelsFilter(float red_scale, float green_scale,
                                      float blue_scale) {
  SaveStateForPossibleUndo();
  f_channels_ = FilterChannels(red_scale, green_scale, blue_scale);
  f_channels_.ApplyToBuffer(current_buffer_);
}

void ImageEditor::ApplyQuantizeFilter(int num_bins) {
  SaveStateForPossibleUndo();
  f_quantize_ = FilterQuantize(num_bins);

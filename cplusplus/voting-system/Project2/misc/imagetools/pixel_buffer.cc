...
void PixelBuffer::SaveToFile(const std::string &filename) {
  ...
  for (int y = 0; y < height_; y++) {
    for (int x = 0; x < width_; x++) {
      color = pixel(x, y);
      image->SetFloatValue(x, y, 0, color.red());
      image->SetFloatValue(x, y, 1, color.green());
      image->SetFloatValue(x, y, 2, color.blue());
      image->SetFloatValue(x, y, 3, color.alpha());
    }
  }
...
void PixelBuffer::LoadFromFile(const std::string &filename) {
  ...
  for (int y = 0; y < image->Height(); y++) {
    for (int x = 0; x < image->Width(); x++) {
      color.set_red(image->FloatValue(x, y, 0));
      color.set_green(image->FloatValue(x, y, 1));
      color.set_blue(image->FloatValue(x, y, 2));
      color.set_alpha(image->FloatValue(x, y, 3));
      set_pixel(x, y, color);
    }
  }
}

...
and parse_compare toks =
  let (lexpr, rest) = parse_addsub toks in             (* No need to loop through expressions, just parse left side on higher order *)
  match rest with
  | GreatThan :: rest ->                               (* Found > *)
    let (rexpr,rest) = parse_addsub rest in            (* Parse right side of expression on higher order *)
    (Intop{op=Greater;lexpr;rexpr}, rest)              (* Create Greater tree *)
  | LessThan :: rest ->                                (* Found < *)
    let (rexpr,rest) = parse_addsub rest in            (* Parse right side of expression on higher order *)
    (Intop{op=Less;lexpr;rexpr}, rest)                 (* Create Less tree *)
  | Equal :: rest ->                                   (* Found = *)
    let (rexpr,rest) = parse_addsub rest in            (* Parse right side of expression on higher order *)
    (Intop{op=Equal;lexpr;rexpr}, rest)                (* Create Equal tree *)
  | _ -> (lexpr, rest)
...
and parse_cond toks =
  ...
  | If :: rest  ->                                     (* If found *)
    begin
      let (if_expr,rest) = parse_expr rest in          (* Parse to define if expression and look for then *)
      match rest with
      | Then :: rest  ->                               (* Then found *)
        begin
          let (then_expr,rest) = parse_expr rest in    (* Parse to define then expression and look for else*)
          match rest with
          | Else :: rest ->                            (* Else found *)
            let (else_expr,rest) = parse_expr rest in  (* Parse to define else expression *)
            (Cond{if_expr; then_expr; else_expr}, rest)(* Create Cond tree using if expression, then expression, and else expression *)
...
and parse_lambda toks =
  match toks with
  | At :: Ident name :: rest ->                        (* Lambda found, make sure followed by Ident name *)
    begin
      let (code_expr,rest) = parse_expr rest in        (* Define the code expression by parsing the rest *)
      (Lambda{param_name=name; code_expr}, rest)       (* Create Lambda tree using Ident name as param_name and code_expr from above *)
    end
  | At :: rest ->
